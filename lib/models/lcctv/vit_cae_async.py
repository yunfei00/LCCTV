...
class Block(nn.Module):
    ...
    def forward(self, x, len_t, feat_val=None):
        # Here we keep the template part unchanged
        x_t = x[:, :len_t]
        x_s = x[:, len_t:]
        ...
        if self.gamma_1 is None:
            ...
            x_s = x_s + self.drop_path(self.gamma_2 *
                                       self.mlp(self.norm2(x_s)))
            x = torch.cat((x_t, x_s), dim=1)
        return x, y_s if feat_val is None else feat_val
...
class VIT_Backbone(nn.Module):
    ...
    def forward_zx(self, z, x, target_token=None):
        ...
        #
        # cls_pos_embed = torch.zeros(1, 1, self.pos_embed.size(-1), device=self.pos_embed.device,
        #                             dtype=self.pos_embed.dtype)
        # pos_embed_with_cls = torch.cat((cls_pos_embed, self.pos_embed_x), dim=1)
        #
        # x = torch.cat((cls_token, x), dim=1)
        # x = self.pos_drop(x + pos_embed_with_cls)
        ...
        for i in range(self.num_FE_layers):
            if i not in self.skip:
                x, features = self.blocks[i](x, 0, feat_val=None)
                # cls_token = features[:, 0, :].unsqueeze(1)
                # features_rest = features[:, 1:, :]
                # cls_token = features[:, 0:1]
                # features_rest = features[:, 1:]  # 除 cls token 外的特征
                features_rest = features
            else:
                if i_param == 0:
                    features_prev = features_rest
                features_cur = self.parametric[i_param](features_prev)
                # x, _ = self.blocks[i](x, 0, feat_val=torch.cat((cls_token, features_cur), dim=1))
                x, _ = self.blocks[i](x, 0, feat_val=features_cur)
        ...
        for i in range(self.num_FE_layers, self.depth):
            # if i not in self.skip:
            #     x, features = self.blocks[i](x, len_z, feat_val=None)
            #     cls_token = features[:, 0, :].unsqueeze(1)
            #     features_rest = features[:, 1:, :]
            # else:
            #     if i_param == 0:
            #         features_prev = features_rest
            #     features_cur = self.parametric[i_param](features_prev)
            #     x, _ = self.blocks[i](x, len_z, feat_val=torch.cat((cls_token, features_cur), dim=1))
            #     features_prev = features_cur
            #     i_param = i_param + 1
            x, _ = self.blocks[i](x, len_z)
            # x = self.blocks[i](x, len_z)
    ...
    #             features_cur = self.parametric[i_param](features_prev)
    #             # x, _ = self.blocks[i](x, 0, feat_val=torch.cat((cls_token, features_cur), dim=1))
    #             x, _ = self.blocks[i](x, len_z, feat_val=features_cur)
    #             features_prev = features_cur
    #             i_param = i_param + 1
    #         # x = self.blocks[i](x, 0)
    #
    ...
    def forward_z(self,
        ...
        for i, blk in enumerate(self.blocks):
            if i not in self.skip:
                z, features = blk(z, 0, feat_val=None)
                # cls_token = features[:, 0, :].unsqueeze(1)
                # features_rest = features[:, 1:, :]
                cls_token = features[:, 0:1]
                features_rest = features[:, 1:]  # 除 cls token 外的特征
                ...
                if i_param == 0:
                    features_prev = features_rest
                features_cur = self.parametric[i_param](features_prev)
                z, _ = blk(z, 0, feat_val=torch.cat((cls_token, features_cur), dim=1))
                # z, _ = blk(z, 0, feat_val=features_cur)
                # feat_input = torch.cat((cls_token, features_cur), dim=1)
                # z, _ = self.blocks[i](z, 0, feat_val=feat_input)
                features_prev = features_cur
                i_param = i_param + 1
            # z, _ = blk(z, 0, feat_val=None)
